...
namespace EmmanuelJavaScriptWeb.Pages.Js_Page
{
    public class IntroductionModel : PageModel
    {
        ...
        public IntroductionModel(ApplicationDbContext dbContext, SignInManager<ApplicationUser> signInManager)
        {
        ...
        public void OnGet()
        {
            if (signInManager.IsSignedIn(User))
            {
                ...
                if (getdata != null)
                {
                    getdata.WebUrl = "/Js-Page/Introduction";
                }
                else
                {
                    Tracker tr = new()
                    {
                        WebUrl = "/Js-Page/Introduction",
                        AppId = claim.Value
                    };
                    dbContext.Add(tr);
                }
